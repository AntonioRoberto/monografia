...
Cada elevador $i$ não consegue atingir andares menores que $Y_i$ e maiores que $F$, ie, $Y_i \leq Y_i+X_it \leq F$, ou melhor, $0 \leq t \leq \frac{F-Y_i}{X_i}$.
...
Dois elevadores $u$ e $v$ tem um andar em comum, se exite inteiros $t_u$ ($0\leq t_u\leq \frac{F-Y_u}{X_u}$) e $t_v$ ($0\leq t_v\leq \frac{F-Y_v}{X_v}$), tal que
$Y_u+X_ut_u = Y_v+X_vt_v$, o que nos dá a \textit{Equação Diofantina Linear} $X_ut_u + (-X_v)tv = (Y_v-Y_u)$.
\\

\textbf{Pseudocódigo:}
\begin{algorithm}
\caption{Skyscraper Floors}\label{euclid}
\begin{algorithmic}[1]
\Procedure{GetMaximumValue (S)}{}
\State $S \gets sort(S)$ \Comment{sort(X) retorna o conjunto X ordenado.} 
\State $maxValue \gets 0$
